...
func SettingsDlgProc(hDlg win.HWND, msg uint32, wParam uintptr, lParam uintptr) uintptr {
	...
	case win.WM_INITDIALOG:
		...
		if s.Settings.SoundMode == settings.SoundMP3 {
	...
	case win.WM_COMMAND:
		...
		case win.IDOK:
			...
			newStyle := settings.ClockDigital
			if isDlgButtonChecked(hDlg, IDC_CLOCK_ANALOG) {
				newStyle = settings.ClockAnalog
			}
			...
			s.Settings.SoundMode = settings.SoundSimple
			if isDlgButtonChecked(hDlg, IDC_SOUND_MP3) {
				s.Settings.SoundMode = settings.SoundMP3
			}
